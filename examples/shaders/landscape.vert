...
float altitude(vec2 coord) {
	...
	float hscale = 1;
	return 0
		+ snoise(coord * nscale * 1) * hscale * 1
		+ snoise(coord * nscale * 2) * hscale * 0.5
		+ snoise(coord * nscale * 4) * hscale * 0.25
		+ snoise(coord * nscale * 8) * hscale * 0.125
		+ snoise(coord * nscale * 16) * hscale * 0.0625
		+ snoise(coord * nscale * 32) * hscale * 0.03125
		+ snoise(coord * nscale * 64) * hscale * 0.015625
		+ snoise(coord * nscale * 128) * hscale * 0.0078125;
}

void main() {
	float value = altitude(u_offset + position.xy);
	vec3 newPos = vec3(position.xy, value * 40);
